...
void FlyingMob::travel(const Vec2& pos)
{
	...
	else
	{
		...
		if (m_bOnGround)
		{
			...
			int var4 = m_pTileSource->getTile(TilePos(Mth::floor(m_pos.x), Mth::floor(m_hitbox.min.y) - 1, Mth::floor(m_pos.z)));
			if (var4 > 0)
			{
		...
		{
			var3 = 546.0f * 0.1f * 0.1f * 0.1f;
			int var5 = m_pTileSource->getTile(TilePos(Mth::floor(m_pos.x), Mth::floor(m_hitbox.min.y) - 1, Mth::floor(m_pos.z)));
			if (var5 > 0)
			{
