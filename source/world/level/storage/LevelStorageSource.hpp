...
struct LevelSummary
{
	...
	LevelSummary(const std::string& fileName, const std::string& levelName, int lastPlayed, int sizeOnDisk, int levelSeed, GameType gameType = GAME_TYPE_CREATIVE, unsigned int storageVersion = LEVEL_STORAGE_VERSION_DEFAULT)
	{
		m_fileName = fileName;
...
public:
	...
	virtual int convertLevel(const std::string&) = 0;
};
