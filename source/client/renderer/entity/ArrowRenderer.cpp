...
void ArrowRenderer::render(const Entity& entity, const Vec3& pos, float rot, float a)
{
    ...
    matrix->rotate(arrow.m_oRot.pitch + (arrow.m_rot.pitch - arrow.m_oRot.pitch) * a - 90.0f, Vec3::UNIT_Y);
    matrix->rotate(arrow.m_oRot.yaw + (arrow.m_rot.yaw - arrow.m_oRot.yaw) * a,               Vec3::UNIT_Z);
